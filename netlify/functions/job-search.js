...
function extractExp(job) {
  ...
  // P1: "X+ years of [adj] experience in/with..." - also handles "X years+" format
  ...
  var m; while ((m = p1.exec(d)) !== null && parts.length < 5) {
    ...
    // Remove "e.g." and parenthetical content from end
    c = c.replace(/\s*\(e\.g\.?.*$/, '').replace(/\s*e\.g\.?.*$/, '');
  ...
  if (parts.length === 0) {
    ...
    var p8 = /(?:including\s*(?:at\s*least\s*)?|plus\s*(?:at\s*least\s*)?|with\s*(?:at\s*least\s*)?|,\s*with\s*)(\d+)(?:\+|\s*plus)?\s*(?:years?|yrs?)['\u2019]?['\u2019]?\s*(?:of\s*)?(?:[\w\s]*?)\s*(?:in|with|as|focused\s*on|focusing\s*on)\s+([\w\s,\/&\-()]+?)(?:\.|;|\n|$|,\s*(?:with|including|and|or))/gi;
    ...
    // P10: "[context] experience (X+ years)" - experience followed by years in parentheses
    var p10 = /([\w\s\/\-]{3,40}?)\s*(?:experience|expertise)\s*\(\s*(\d+)(?:\+|\s*plus)?\s*(?:years?|yrs?)\s*\)/gi;
    while ((m = p10.exec(d)) !== null && parts.length < 6) {
      var y10 = parseInt(m[2]);
      if (y10 < 1 || y10 > 30) continue;
      var c10 = m[1].trim().replace(/^\s*(?:a|an|the|and|or|with|proven|strong|solid|relevant|demonstrated)\s+/gi, '').replace(/\s+$/,'');
      if (c10.length < 3) continue;
      var k10 = c10.substring(0,25).toLowerCase();
      if (!seen[k10]) { seen[k10] = true; parts.push(y10+'+ yr '+c10); }
    }
  }
  if (parts.length === 0) {
...
function extractSalary(job) {
  ...
  if (m3) {
    ...
    if (m3[3]) {
      ...
      sfx3 = detectPeriod(d, m3[0]) || '';
      // #304: Smart default based on amounts — small amounts are hourly
      if (!sfx3) {
        var amt1 = parseFloat(m3[1].replace(/,/g, '')), amt2 = parseFloat(m3[2].replace(/,/g, ''));
        if (amt1 < 500 && amt2 < 500) sfx3 = '/hr';
        else if (amt1 >= 1000 && amt2 >= 1000) sfx3 = '/yr';
        else sfx3 = '/yr';
      }
    }
...
function extractContact(job) {
  ...
  var emailMatch = d.match(/(?:contact|email|reach|send|apply|submit|inquir)\w*\s*(?:me\s*)?(?:directly\s*)?(?:at|to|via|through|:)\s*([\w.\-+]+@[\w.\-]+\.[\w]{2,})/i);
  if (!emailMatch) {
...
function detectCountry(job, searchCountry) {
  ...
  var loc = [job.job_city, job.job_state, job.job_country, job.location].filter(Boolean).join(', ');
  ...
  // #312: Direct full country-name match in location string (e.g. "Abu Dhabi Emirate, United Arab Emirates")
  var countryNames = ['United Arab Emirates','United Kingdom','United States','Saudi Arabia','South Africa','South Korea','New Zealand','Hong Kong','Czech Republic','Netherlands','Switzerland','Singapore','Australia','Germany','Indonesia','Philippines','Malaysia','Argentina','Colombia','Pakistan','Portugal','Romania','Belgium','Bahrain','Canada','Ireland','Sweden','Israel','Brazil','Mexico','Norway','Denmark','Finland','Hungary','Austria','Türkiye','Turkey','Ukraine','Vietnam','Thailand','Taiwan','Nigeria','Kuwait','Qatar','France','Poland','Greece','Spain','Italy','Japan','China','India','Egypt','Kenya','Chile','Peru','Oman'];
  for (var cn = 0; cn < countryNames.length; cn++) {
    var re = new RegExp(countryNames[cn].replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
    if (re.test(loc)) return countryNames[cn] === 'Turkey' ? 'Türkiye' : countryNames[cn];
  }
  ...
  var locStr = [job.job_city, job.job_state, job.job_country, job.location].filter(Boolean).join(', ');
...
function classifyCompany(company, desc, companyUrl, defaultType) {
  ...
  // Priority 0: URL signals job board/careers site (e.g. confidential.careers, *.jobs)
  if (url) {
    if (/\b(?:confidential\.careers|\.careers\/?$|\/careers\.|jobs\.|\.jobs\b|careers\.|recruiting\.|talent\.|apply\.|jobboard|job-board)\b/i.test(url)) {
      // careers subdomain of a real company isn't necessarily a job board, but standalone careers/jobs domains are
      if (/(?:^https?:\/\/)?(?:www\.)?(?:confidential\.careers|[\w-]+\.jobs)\b/i.test(url) || /confidential/i.test(url)) return 'Job Board';
    }
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      var BAD_CERTS = /^(?:cka|ckad|cks|zero\s*trust)$/i;
      jobs.forEach(function(j) {
      ...
      // #282: Save extracted emails as contacts
      try {
        var contactsCol = db.collection('contacts');
        var contactOps = [];
        jobs.forEach(function(j) {
          if (!j.contact || j.contact === 'See details' || j.contact === 'Search LinkedIn') return;
          var emailMatch = j.contact.match(/([\w.+-]+@[\w.-]+\.[a-z]{2,})/i);
          if (!emailMatch) return;
          var email = emailMatch[1];
          // Extract name from "Name (email)" format or from email itself
          var name = '';
          var nameMatch = j.contact.match(/^([A-Z][a-z]+\s+[A-Z][a-z]+)/);
          if (nameMatch) name = nameMatch[1];
          if (!name) {
            var local = email.split('@')[0].replace(/[._-]+/g, ' ').split(' ').filter(function(p) { return p.length > 1; });
            if (local.length >= 2) name = local.map(function(p) { return p.charAt(0).toUpperCase() + p.slice(1).toLowerCase(); }).join(' ');
          }
          contactOps.push({
            updateOne: {
              filter: { email: email, company: { $regex: j.company.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' } },
              update: { $setOnInsert: { name: name || 'Unknown', email: email, designation: 'Contact (from JD)', linkedin: '', company: j.company, source: 'Job Description', createdAt: new Date() } },
              upsert: true
            }
          });
        });
        if (contactOps.length > 0) await contactsCol.bulkWrite(contactOps, { ordered: false });
      } catch (contactErr) { console.error('Contact save error:', contactErr.message); }
    } catch (dbErr) {
