...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (action === 'draftOutreach') {
      ...
      // #552/#554: one AI generation per job per kind, cached on the job document (jobId or _id)
      var { getDb: gdbO } = require('./db');
      var odb = await gdbO();
      var oField = kind === 'client' ? 'outreachClient' : 'outreachConsultant';
      var oQuery = null;
      if (oj.jobId) {
        oQuery = { jobId: oj.jobId };
        if (/^[0-9a-f]{24}$/i.test(String(oj.jobId))) { try { var OID = require('mongodb').ObjectId; oQuery = { $or: [{ jobId: oj.jobId }, { _id: new OID(String(oj.jobId)) }] }; } catch (qe) {} }
      }
      var ojDoc = oQuery ? await odb.collection('jobs').findOne(oQuery, { projection: { _id: 1, outreachClient: 1, outreachConsultant: 1 } }) : null;
      if (ojDoc && ojDoc[oField] && ojDoc[oField].body && !body.force) {
        return { statusCode: 200, headers: hdrs, body: JSON.stringify({ subject: ojDoc[oField].subject, html: ojDoc[oField].html, body: ojDoc[oField].body, cached: true }) };
      }
      var jobUS = /united states|usa/i.test(String(oj.detectedCountry || oj.country || ''));
      var consIndia = /india/i.test(String(op.country || ''));
      var SIG = 'Signature block, exactly these lines: "Sunil Shilimkar"; "Business Head | SecVision Technologies LLP | Pune | India"; then "sunil@secvisiontech.com | secvisiontech.com" where sunil@secvisiontech.com is a mailto: hyperlink (href mailto:sunil@secvisiontech.com) and secvisiontech.com is a hyperlink to https://secvisiontech.com/ - each linked to its OWN target, never cross-linked (both colored #0E7490). Nothing after these lines - no separate Pune, India line.';
      var STYLE = 'OUTPUT FORMAT: first line "SUBJECT: <subject>". Then a complete HTML email body (no <html>/<head>, start at a wrapper <div>) with inline CSS only: font-family Arial/Helvetica; a header line with "SecVision Technologies" in bold color #0E7490; the requirement as a bordered table (th left-aligned #F0F9FF background, 1px #CBD5E1 borders, compact padding) with ONLY populated fields; short paragraphs; numbered questions as an <ol>. Professional, concise. After the HTML, output a line "---PLAIN---" followed by the same email as clean plain text.';
      var oPrompt = kind === 'client'
        ? 'Write a professional business email from Sunil Shilimkar, Business Head, SecVision Technologies LLP (Pune-based cybersecurity services company; leadership: Microsoft Certified Cybersecurity Architect Expert, 18+ years, 2 US patents; clients in USA/Europe) to the company that posted this staffing requirement. IMPORTANT CONTEXT RULES: (a0) If the data has a jobUrl, add immediately BEFORE the requirement table one short line: Reference - we are writing regarding your posting at <a href=THE_URL>the original job posting</a> (hyperlink it, color #0E7490) so the client knows which posting this email concerns; omit when no jobUrl. (a) We CAME ACROSS this posted requirement ourselves (we were not sent it) - open accordingly, e.g. "We came across your <role> requirement and it aligns closely with the consultants we place." Never thank them for sharing it. (b) Greeting: "Hello Hiring Team (' + (oj.company || 'Team') + ')," followed on the next line by "Greetings !!" - short, no "Dear Hiring Manager at...". (c) Work-mode consistency: if the data says On-site, do NOT ask an open remote-vs-onsite question; instead ask whether the client would CONSIDER remote delivery from India as an alternative to the stated on-site mode. If mode is Remote or unknown, ask to confirm remote-from-India acceptability. (d) Ask numbered clarification questions ONLY for genuinely missing items among: offered hourly rate' + (jobUS ? ' (and C2C/W2 basis - US engagement)' : '') + ', work-mode per rule (c), working-hours/shift overlap, contract duration and start date. NEVER assert that we already have matching/qualified candidates on hand - use conditional sourcing language only (for example: we could identify qualified candidates with the required skills; our network covers consultants with this profile). Then: one line offering matched consultant profiles once details are confirmed; one warm line about building a long-term partnership with ' + (oj.company || 'the company') + '. ' + SIG + ' ' + STYLE + ' JOB DATA: '
        : 'Write a professional, warm outreach email from Sunil Shilimkar, Business Head, SecVision Technologies LLP to a security consultant about a live contract requirement. Greeting: exactly "Hello {{FirstName}}," (keep the literal placeholder {{FirstName}} - substituted per consultant later) followed on the next line by "Greetings !!". Frame SecVision as placing security consultants on remote contract engagements with US/EU clients. Include: one-line intro; the requirement table with Job ID as the FIRST row (value exactly: ' + JSON.stringify(String(oj.jobId || '')) + ' - include verbatim so future correspondence can reference it), then Role, Technology, Skills, Experience, Certifications, Compliance - only populated fields, skip See-details values; numbered questions: current availability - specifically when their current engagement ends or from when they can start (contractors rarely have a formal notice period, so ask it that way); expected rate (USD/hr, contract), phrased as: if not already shared with us; ' + (jobUS ? 'flexibility for US-hours overlap (for India-based consultants: evening/night shift IST);' : 'flexibility for working-hours overlap with the client timezone;') + ' then close with exactly two short points in plain words: there are no fees to the consultant at any stage; and each engagement is a dedicated, full-time commitment for its duration. Do NOT use the words exclusive/non-exclusive/lock-in, and do NOT say they are free to take other work. ' + SIG + ' ' + STYLE + ' JOB DATA: ';
      var oData = JSON.stringify({ job: { title: oj.title, company: oj.company, tools: oj.tools, skills: oj.skills, experience: oj.experience, certifications: oj.certifications, compliance: oj.compliance, location: oj.location, remote: oj.remote, salary: oj.salary, jobUrl: oj.jobUrl || oj.applyLink || '', duration: oj.contractDuration, country: oj.detectedCountry || oj.country, description_hints: String(oj.description || '').slice(0, 1200) }, consultant: kind === 'consultant' ? { name: op.name, country: op.country, rate: op.rateExpectation || '' } : undefined });
      try {
        var orr = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': OKEY, 'anthropic-version': '2023-06-01' },
          body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 2400, messages: [{ role: 'user', content: oPrompt + oData + '\n\nAlso output a subject line as the FIRST line prefixed "SUBJECT: ".' }] }) });
        ...
        var subj = '', htmlB = '', plainB = '';
        var sm = oout.match(/^SUBJECT:\s*(.+)$/m);
        if (sm) { subj = sm[1].trim(); oout = oout.replace(/^SUBJECT:.*$/m, '').trim(); }
        var parts = oout.split(/---PLAIN---/);
        htmlB = (parts[0] || '').trim();
        plainB = (parts[1] || '').trim() || htmlB.replace(/<[^>]+>/g, ' ').replace(/\s{2,}/g, ' ').trim();
        var persistedOK = false;
        try {
          if (ojDoc && ojDoc._id) { var ur = await odb.collection('jobs').updateOne({ _id: ojDoc._id }, { $set: { [oField]: { subject: subj, html: htmlB, body: plainB, at: new Date() } } }); persistedOK = ur.matchedCount > 0; }
          else if (oQuery) { var ur2 = await odb.collection('jobs').updateOne(oQuery, { $set: { [oField]: { subject: subj, html: htmlB, body: plainB, at: new Date() } } }); persistedOK = ur2.matchedCount > 0; }
        } catch (pe) {}
        return { statusCode: 200, headers: hdrs, body: JSON.stringify({ subject: subj, html: htmlB, body: plainB, cached: false, persisted: persistedOK }) };
