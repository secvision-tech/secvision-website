  ...
  try {
    ...
    if (action === 'draftOutreach') {
      ...
      var oPrompt = kind === 'client'
        ...
        : 'Write a professional, warm outreach email from Sunil Shilimkar, Business Head, SecVision Technologies LLP to a security consultant about a live contract requirement. Greeting: "Dear <FirstName>," followed on the next line by "Greetings !!". ' + (consIndia ? 'The consultant is India-based: frame SecVision as placing India-based security consultants on remote US/EU contract engagements.' : 'The consultant is outside India: frame SecVision as engaging independent security consultants worldwide for remote US/EU contract engagements.') + ' Include: one-line intro; the requirement table (Role, Technology, Skills, Experience, Certifications, Compliance - only populated fields, skip "See details"); numbered questions: current availability - specifically when their current engagement ends or from when they can start (contractors rarely have a formal notice period, so ask it that way); expected rate (USD/hr, contract) ONLY if the profile has no rate; ' + (consIndia && jobUS ? 'flexibility for US-hours overlap (evening/night shift IST);' : 'flexibility for working-hours overlap with the client timezone;') + ' then close with exactly two short points in plain words: there are no fees to the consultant at any stage; and each engagement is a dedicated, full-time commitment for its duration. Do NOT use the words exclusive/non-exclusive/lock-in, and do NOT say they are free to take other work. ' + SIG + ' ' + STYLE + ' JOB DATA: ';
