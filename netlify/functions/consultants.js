...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (action === 'translateProfiles') {
      ...
      try {
        var tOut = await require('./translate-fields').translateProfilesNonLatin(col, tIds);
        return { statusCode: 200, headers: hdrs, body: JSON.stringify({ translated: tOut }) };
      } catch (e) { return { statusCode: 200, headers: hdrs, body: JSON.stringify({ translated: {}, error: 'Translate failed: ' + e.message }) }; }
    }
